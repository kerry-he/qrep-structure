...
mutable struct QuantCoherentInformation{T <: Real} <: Hypatia.Cones.Cone{T}
    ...
    function QuantCoherentInformation{T}(
        ni::Int,
        no::Int,
        ne::Int,
        N::Function,
        ...
        cone.N  = lin_to_mat(T, x -> N(x), ni, no)         # Quantum channel
        cone.Nc = lin_to_mat(T, x -> Nc(x), ni, ne)        # Complementary channel

        return cone
    end
end

function Hypatia.Cones.reset_data(cone::QuantCoherentInformation)
    return (
        cone.feas_updated =
            cone.grad_updated =
                cone.hess_updated =
...
function Hypatia.Cones.setup_extra_data!(
    cone::QuantCoherentInformation{T},
) where {T <: Real}
    ni = cone.ni
    no = cone.no
    ne = cone.ne
...
Hypatia.Cones.get_nu(cone::QuantCoherentInformation) = 1 + cone.ni
...
function Hypatia.Cones.update_feas(cone::QuantCoherentInformation{T}) where {T <: Real}
    ...
    if isposdef(XH)
        ...
        if isposdef(X_fact) && isposdef(NX_fact) && isposdef(NcX_fact)
            ...
            cone.z = point[1] - (entr_NX - entr_NcX)
...
function Hypatia.Cones.update_grad(cone::QuantCoherentInformation)
    @assert cone.is_feas
    rt2 = cone.rt2
    (Λx, Ux) = cone.X_fact
    Xi = cone.Xi
...
function update_hessprod_aux(cone::QuantCoherentInformation)
...
function update_invhessprod_aux(cone::QuantCoherentInformation)
    ...
    # Hessian for -logdet(X) component
    k = 1
    @inbounds for j in 1:cone.ni, i in 1:j
        XHX = cone.Xi[i, :] * cone.Xi[j, :]'
        if i != j
            XHX = (XHX + XHX') / rt2
        end
        @views Hypatia.Cones.smat_to_svec!(cone.hess[:, k], XHX, cone.rt2)
        k += 1
    end
...
function update_dder3_aux(cone::QuantCoherentInformation)
...
function Hypatia.Cones.dder3(cone::QuantCoherentInformation{T}, dir::AbstractVector{T}) where {T <: Real}
    @assert cone.grad_updated
    cone.hessprod_aux_updated || update_hessprod_aux(cone)
